...
Public Class Host
    ...
    Public Shared Function GetNewInspectorItem(<CallerMemberName> Optional memberName As String = "", <CallerFilePath> Optional fileName As String = "", <CallerLineNumber> Optional lineNumber As Integer = 0) As InspectorItem
        ...
        If GlobalSettings.Settings.InspectorEnabled Then
            Return New Inspector.InspectorItem With {.CodePath = (fileName & "#L" & lineNumber).Replace(fileName.Substring(0, Directory.GetParent(Directory.GetParent(fileName).FullName).FullName.Length), "https://github.com/DanWBR/dwsim5/blob/master").Replace("\", "/")}
        Else
            Return Nothing
        End If
